...
class Curve(Serializable):
    ...
    def to_dict(self):
        """Return a dict representation of the curve.

        Returns
        -------
        dict
            Dict with a "points" key mapping to the dict representation
            (:meth:`ccp.Point.to_dict`) of each point in the curve.
        """
        return {
            "points": {f"point{i}": point.to_dict() for i, point in enumerate(self)}
        }

    @classmethod
    def from_dict(cls, dict_parameters):
        """Create a curve from a dict created with :meth:`to_dict`.
