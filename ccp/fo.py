...
class FlowOrifice:
    def __init__(self, state=None, delta_p=None, qm=None, D=None,
                 tappings='flange'):
        self.state = state
        ...
        d = newton(self.calc_d, 0.1,
                   args=(
                   self.state, self.delta_p, self.qm, self.D, self.tappings))
        self.d = Q_(float(d), 'm')

    def __setattr__(self, name, value):
        try:
            if (
                isinstance(getattr(self, name), Q_)
                and not
                isinstance(value, Q_)
            ):
            ...
            d = newton(self.calc_d, 0.1,
                       args=(self.state, self.delta_p, self.qm, self.D,
                             self.tappings))
            super().__setattr__('d', Q_(float(d), 'm'))
    ...
    def calc_d(x, state=None, delta_p=None, qm=None, D=None, tappings='flange'):
        ...
        C = (
            ...
            + (0.0188 + 0.0063
               * (19000 * beta / reyn)**0.8) * beta ** 3.5 * (1e6 / reyn) ** 0.3
        ...
        # calc e
        e = (
             1 - (0.351 + 0.256 * beta ** 4 + 0.93 * beta ** 8)
             * (1 - (p2 / p1) ** (1 / state.kT()))
        )
