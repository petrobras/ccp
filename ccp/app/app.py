...
for s in sources:
    ...
    bokeh_sources[s] = r_getattr(imp, s + "_bokeh_source")(speed_units=speed_units)
    bokeh_figures[s] = figure(plot_width=500, plot_height=250)

    #  first plot
    bokeh_figures[s] = r_getattr(imp.current_curve, s + "_bokeh_plot")(
        fig=bokeh_figures[s], speed_units=speed_units, plot_kws=dict(color="red")
    )
    bokeh_figures[s] = r_getattr(imp, s + "_bokeh_plot")(
        fig=bokeh_figures[s], speed_units=speed_units, plot_kws=dict(color="red")
    )

    bokeh_figures[s] = r_getattr(imp.new.current_point, s + "_bokeh_plot")(
        fig=bokeh_figures[s], source=bokeh_current_point_sources[s],
    )
    bokeh_figures[s] = r_getattr(imp.new.current_curve, s + "_bokeh_plot")(
        fig=bokeh_figures[s],
        source=bokeh_current_curve_sources[s],
        speed_units=speed_units,
    )
    bokeh_figures[s] = r_getattr(imp.new, s + "_bokeh_plot")(
...
def update_fig(attr, old, new):
    if Ts.value != imp.new.suc.T().magnitude or ps.value != imp.new.suc.p().magnitude:
        new_suc = State.define(p=ps.value, T=Ts.value, fluid=imp.new.suc.fluid)
        imp.suc = new_suc

    imp.new.flow_v = flow_v.value
    imp.new.speed = Q_(sp.value, speed_units)

    speeds = [curve.speed.to(speed_units).magnitude for curve in imp.new.curves]
...
flow_v = Slider(
    title="Suction flow (m³/s)",
    value=imp.new.flow_v.magnitude,
    start=flow_v_start,
    end=flow_v_end,
    step=flow_v_step,
)
...
button.callback = CustomJS(
    args=dict(source=download_source), code=open(str(Path.cwd() / "download.js")).read()
)
