...
df = pd.read_csv(CCP_PATH / 'tests/data/UTGCA_1231_A_2019-01-01T00_00_00_2020-12-11T00_00_00_12h')
...
def calculate_performance(tag, sample):
    ...
    flow_v_st = Q_(getattr(data, f'df{tag}')[tags_dict[f'tags_{tag}'][4]][sample], 'm**3/h')
    speed_st = Q_(getattr(data, f'df{tag}')[tags_dict[f'tags_{tag}'][5]][sample], 'rpm')

    composition_st = dict(
        methane=getattr(data, f'df{tag}').UTGCA_1231_AI_002_C1[sample],  # Analisador_METANO_AW-002',
        ethane=getattr(data, f'df{tag}').UTGCA_1231_AI_002_C2[sample],  # Analisador_ETANO_AW-002',
        propane=getattr(data, f'df{tag}').UTGCA_1231_AI_002_C3[sample],  # CPR_Analisador_PROPANO_AW-002',
        hexane=getattr(data, f'df{tag}').UTGCA_1231_AI_002_C6[sample],  # Analisador_HEXANO_AW-002',
        co2=getattr(data, f'df{tag}').UTGCA_1231_AI_002_CO2[sample],  # Analisador_GÁS CARBÔNICO_AW-002',
        ibutane=getattr(data, f'df{tag}').UTGCA_1231_AI_002_IC4[sample],  # Analisador_I-BUTANO_AW-002',
        ipentane=getattr(data, f'df{tag}').UTGCA_1231_AI_002_IC5[sample],  # Analisador_I-PENTANO_AW-002',
        nitrogen=getattr(data, f'df{tag}').UTGCA_1231_AI_002_N2[sample],  # Analisador_NITROGÊNIO_AW-002',
        nbutane=getattr(data, f'df{tag}').UTGCA_1231_AI_002_NC4[sample],  # Analisador_N-BUTANO_AW-002',
        npentane=getattr(data, f'df{tag}').UTGCA_1231_AI_002_NC5[sample],  # Analisador_N-PENTANO_AW-002',
    )
...
def generate_graphs(n):
    ...
    fig.for_each_trace(lambda trace: trace.update(name='Expected Point') if trace.name == 'Current Point' else ())
    atual = fig
