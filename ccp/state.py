        ...
        for i in self.isolines:
            ...
            if i == CoolProp.iQ:
                ...
                if dew is not None and bub is not None:
                    ...
                    if (
                        filter_x(dew.x[dew_filter][-1])
                        - filter_x(bub.x[bub_filter][-1])
                    ) < 0.050 * filter_x(dx) or (
                        filter_y(dew.y[dew_filter][-1])
                        - filter_y(bub.y[bub_filter][-1])
                    ) < 0.010 * filter_y(
                        dy
                    ):
                        x = np.linspace(
                            bub.x[bub_filter][-1], dew.x[dew_filter][-1], 11
                        )
                        try:
                            ...
                            self.axis.plot(
                                dim_x.from_SI(x), dim_y.from_SI(y), **sat_props
                            )
                            warnings.warn(
                                "Detected an incomplete phase envelope, fixing it numerically."
                            )
                            x_crit = x[5]
                            y_crit = y[5]
                        except ValueError:
            ...
            for line in self.isolines[i]:
                if line.i_index == CoolProp.iQ:
                    if line.value == 0.0 or line.value == 1.0:
                        self.axis.plot(
                            dim_x.from_SI(line.x), dim_y.from_SI(line.y), **sat_props
                        )
                    else:
