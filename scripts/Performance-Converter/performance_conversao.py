...
if __name__ == "__main__":
    ...
    def gas_composition(
        p, T, column=3, sheet=o_sheet, o_sheet_template=True, output_sheet=True
    ):
        ...
        for i in range(25):
            if sheet.cells(12 + i - nn, column).value == None:
                ...
                N = 25
        Gases = sheet.range(
            sheet.cells(12 - nn, column), sheet.cells(12 - nn + N - 1, column)
        ).value
        mol_frac = sheet.range(
            sheet.cells(12 - nn, column + 2), sheet.cells(12 - nn + N - 1, column + 2)
        ).value
    ...
    def load_from_excel(
        speed_unit, var1_unit, var2_unit, cell_speed_n="D40", column=3, sheet=o_sheet
    ):
    ...
    def array_from_csv(
        suc, curve_cell, b, D, flow_unit, var1_unit, speed_unit, n_imp=[0]
    ):  # "L12"
        ...
        try:
            imp = ccp.Impeller.load_from_engauge_csv(
                ...
                number_of_points=N_lines,
                curve_path=data_dir,
                b=b,
                D=D,
                flow_units=flow_unit,
                head_units=var1_unit,
        ...
        dt = tf - t0
        print(
            f"Tempo criando impelidor {n_imp[0]}: "
            + str(int(dt / 60))
            + " minutos e "
            + str(int(dt % 60))
            + " segundos"
    ...
    def impeller_from_excel(N_speed, speed_array, Qs, H, Eff, n_imp=[0]):
        ...
        dt = tf - t0
        print(
            f"Tempo armazenando pontos do impelidor {n_imp[0]}: {int(dt / 60)} minutos e {int(dt % 60)} segundos"
        )
        ...
        dt = tf - t0
        print(
            f"Tempo criando impelidor {n_imp[0]}: {int(dt / 60)} minutos e {int(dt % 60)} segundos"
        )
    ...
    def convert_and_store(
        ...
        dt = tf - t0
        print(
            f"Tempo convertendo pontos do impelidor {n_imp[0]}: {int(dt / 60)} minutos e {int(dt % 60)} segundos"
        )
        ...
        for j in range(N_speed):
            ...
            for i in range(N_lines):
                ...
                if pr:
                    H_n[i, j] = (
                        imp_conv.curves[l][i].disch.p().m
                        / imp_conv.curves[l][i].suc.p().m
                    )
                else:
    ...
    def convert_speeds(speed_n_n, flow_unit_out, var1_unit_out, var2_unit_out, *args):
        ...
        if len(speed_n.m) > 1:
            for i in range(N_speed_n_n):
                closest_curves_idxs = ccp.impeller.find_closest_speeds(
                    speed_n.m, speed_n_n[i].m
                )
                ...
                for j in range(number_of_points):
                    flow_H, H_n_n[j, i] = ccp.impeller.get_interpolated_values(
                        factor,
                    ...
                    flow_Eff, Eff_n_n[j, i] = ccp.impeller.get_interpolated_values(
                        factor,
                        flow_0=Qs_n[j, idx_l].m,
    ...
    def convert_speeds_ccp(
        speed_n_n, flow_unit_out, var1_unit_out, var2_unit_out, imp_n, pr=False
    ):
        # create an impeller that will hold and convert curves.
        ...
        dt = tf - t0
        print(
            f"Tempo convertendo curvas para velocidades desejadas: \n {int(dt / 60)} minutos e {int(dt % 60)} segundos"
        )
    ...
    while Open_file:
        ...
        if o_sheet["K12"].value == "on":
            ...
            if MW_question == "NÃO":
                ...
                if case2_question == "SIM":
                    suc2_o, MW2_o, km2_o, Zm2_o, gas_constant = gas_composition(
                        p=Ps2_o, T=Ts2_o, column=23
                    )

                if v_sheet["K12"].value == "on":
                    suc_v, MW_v, km_v, Zm_v, gas_constant = gas_composition(
                        p=Ps_v, T=Ts_o, sheet=v_sheet
                    )
            ...
            if excel_question == "excel":
                #######################################################################################################################
                N_speed, N_lines, Qs_o, H_o, Eff_o, speed_o = load_from_excel(
                    speed_unit, var1_unit, var2_unit, cell_speed_n="D40", column=3
                )
                if case2_question == "SIM":
                    N2_speed, N2_lines, Qs2_o, H2_o, Eff2_o, speed2_o = load_from_excel(
                        speed_unit, var1_unit, var2_unit, cell_speed_n="X40", column=23
                    )
                    case2 = True

            #######################################################################################################################
            else:
                imp, N_speed, N_lines, Qs_o, H_o, Eff_o, speed_o = array_from_csv(
                    suc_o, "E2", b, D, flow_unit, var1_unit, speed_unit
                )
                if case2_question == "SIM":
                    (
                        imp2,
                        ...
                        suc2_o, "E3", b, D, flow_unit, var1_unit, speed_unit
                    )
                    case2 = True

            if v_sheet["K12"].value == "on":
                if excel_questionv == "excel":
                    N_speedv, N_linesv, Qs_v, H_v, Eff_v, speed_v = load_from_excel(
                        speed_unit,
                        var1_unit,
                        var2_unit,
                        cell_speed_n="D40",
                    ...
                    (
                        ...
                        suc_v, "I2", b, D, flow_unit, var1_unit, speed_unit
                    )
                val_data = True
            ...
            if simp_question == "SIM" or MW_question == "NÃO":
                ...
                np_o = Q_(
                    ...
                    * km_o.magnitude
                    / (
                        km_o.magnitude * Eff_o.magnitude
                        + np.ones([N_lines, N_speed]) * (1 - km_o.magnitude)
                    ),
                    "dimensionless",
                )
                vr_o = (
                    H_o * (np_o - 1) / np_o * MW_o / (Zm_o * gas_constant * Ts_o) + 1
                ) ** (1 / (np_o - 1))
                vr_o = vr_o.to("dimensionless")
                case2 = False
            elif excel_question == "excel":
                imp = impeller_from_excel(
                    N_speed, speed_array=speed_o, Qs=Qs_o, H=H_o, Eff=Eff_o
                )

                if case2_question == "SIM":
                    imp2 = impeller_from_excel(
                        N_speed=N2_speed,
                        speed_array=speed2_o,
                        Qs=Qs2_o,
                        H=H2_o,
                        Eff=Eff2_o,
                    )
        ...
        if n_sheet["I5"].value == "on" and o_sheet["K15"].value == "Dados Carregados":
            ...
            try:
                Ps_n = Q_(n_sheet["F5"].value, n_sheet["G5"].value).to("Pa") + Q_(
                    1, "atm"
                ).to("Pa")
            ...
            if MW_question == "NÃO":
                ...
                suc_n, MW_n, km_n, Zm_n, gas_constant = gas_composition(
                    p=Ps_n, T=Ts_n, sheet=n_sheet, o_sheet_template=False
                )
            ...
            if n_sheet["J17"].value != None:
                ...
                speeds_array = np.array(
                    n_sheet.range((17, 10), (17 + NT - 1, 10)).value
                )
                if NT > 1:
            ...
            if simp_question == "SIM" or MW_question == "NÃO":
                try:
                    ...
                    vr_o = (
                        H_o * (np_o - 1) / np_o * MW_o / (Zm_o * gas_constant * Ts_o)
                        + 1
                    ) ** (1 / (np_o - 1))
                    vr_o = vr_o.to("dimensionless")
                ...
                np_n = (
                    Eff_n
                    * km_n
                    / (km_n * Eff_n + np.ones([N_lines, N_speed]) * (1 - km_n))
                )
                H_n = (
                    np_n
                    / (np_n - 1)
                ...
                for i in range(N_speed):
                    speed_n_matrix[:, i] = speed_o[i] * (H_n[:, i] / H_o[:, i]) ** (
                        1 / 2
                    )
                ...
                # New speed values
                speed_n = Q_(
                    np.mean(speed_n_matrix.magnitude, axis=0), speed_n_matrix.units
                )

            else:
                H_n, Qs_n, Eff_n, speed_n, imp_n = convert_and_store(
                    imp,
                    N_speed,
                    N_lines,
                    flow_unit_out,
                ...
                if case2_question == "SIM" and case2:
                    H2_n, Qs2_n, Eff2_n, speed2_n, imp2_n = convert_and_store(
                        imp2,
                        N2_speed,
                        N2_lines,
                        flow_unit_out,
            ...
            if simp_question == "SIM" or MW_question == "NÃO":
                ...
                if case2_question == "SIM" and case2:
                    ...
                    dt = tf - t0
                    print(
                        f"Tempo consolidando curvas: {int(dt/60)} minutos e {int(dt%60)} segundos."
                    )
                ...
                (
                    ...
                    speed_n_n, flow_unit_out, var1_unit_out, var2_unit_out, imp_n, pr=pr
                )
        ...
        if CF_sheet["O3"].value == "on" and n_sheet["E8"].value != "NÃO":
            ...
            T_normal = Q_(0, "degC")
            try:
                disch_normal_n, _, _, _, gas_constant = gas_composition(
                    p=P_normal,
                    T=T_normal,
                    sheet=n_sheet,
                ...
                disch_normal_o, _, _, _, _ = gas_composition(
                    p=P_normal, T=T_normal, output_sheet=False
                )
            ...
            if NT != 0:
                ...
                for i in range(NT):
                    if CF_sheet["O7"].value == "Vazão Calculada":
                        t_sheet.cells(20, 5 + i).value = (
                            Q_(
                                CF_sheet.cells(5 + i, 10).value,
                                CF_sheet.cells(4, 10).value,
                            )
                            .to(t_sheet.cells(20, 4).value)
                            .magnitude
                        )
                    Ps_T = Q_(
                        t_sheet.cells(5, 5 + i).value, t_sheet.cells(5, 4).value
                    ) + Q_(1, "atm").to("Pa")
                    Ts_T = Q_(t_sheet.cells(6, 5 + i).value, t_sheet.cells(6, 4).value)
                    Pd_T = Q_(
                        t_sheet.cells(7, 5 + i).value, t_sheet.cells(7, 4).value
                    ) + Q_(1, "atm").to("Pa")
                    ...
                    if gas == "Original":
                        ...
                        disch_normal = disch_normal_o
                    try:
                        disch_normal = ccp.State(
                            p=Q_(1, "atm"), T=Q_(0, "degC"), fluid=fluid
                        )
                    except ValueError:
                    ...
                    if t_sheet.cells(20, 5 + i).value != None:
                        flow_m = Q_(
                            t_sheet.cells(20, 5 + i).value, t_sheet.cells(20, 4).value
                        )
                        flow_normal = (
                            disch_normal.z()
                            * disch_normal.gas_constant()
                            * Q_(0, "degC").to("kelvin")
                            * flow_m
                            / (disch_normal.molar_mass() * Q_(1, "atm"))
                        )
                        t_sheet.cells(22, 5 + i).value = flow_normal.to(
                            t_sheet.cells(22, 4).value
                        ).magnitude
                    elif t_sheet.cells(9, 5 + i).value == None:
                        t_sheet["B31"].value = "Vazão do Gás!"
                        t_sheet["B32"].value = "Reinicie o Script!"
                        raise ValueError(
                            "Mass flow or Normal flow rate must be provided!"
                        )
                    else:
                        flow_m = (
                            Q_(1, "atm")
                            * Q_(
                                t_sheet.cells(9, 5 + i).value, t_sheet.cells(9, 4).value
                            )
                            * disch_normal.molar_mass()
                            / (
                                disch_normal.z()
                                * disch_normal.gas_constant()
                                * Q_(0, "degC").to("kelvin")
                        ...
                        t_sheet.cells(20, 5 + i).value = flow_m.to(
                            t_sheet.cells(20, 4).value
                        ).magnitude

                    speed_T = Q_(
                        t_sheet.cells(12, 5 + i).value, t_sheet.cells(12, 4).value
                    )
                ...
                for i in range(NT):
                    t_sheet.cells(14, 5 + i).value = (
                        P_test[i].head.to(t_sheet.cells(14, 4).value).magnitude
                    )
                    t_sheet.cells(15, 5 + i).value = P_test[i].eff.magnitude * 100
                    t_sheet.cells(16, 5 + i).value = (
                        P_test[i].power.to(t_sheet.cells(16, 4).value).magnitude
                    )
                    t_sheet.cells(21, 5 + i).value = (
                        P_test[i].flow_v.to(t_sheet.cells(21, 4).value).magnitude
                    )
        ...
        if (
            n_sheet["N5"].value == "on"
            and o_sheet["K15"].value == "Dados Carregados"
            and n_sheet["I8"].value == "Curvas Convertidas"
        ):
            n_sheet.api.Unprotect()
            ...
            if (
                t_sheet.range("C18").value == "SIM"
                and t_sheet["B31"].value == "Performance Calculada"
            ):
            ...
            for j in range(N_speed_n_n):
                ...
                # count +=1
                count += 1
                n_sheet.cells(count, 3).value = round(
                    speed_n_n[j].to(speed_unit_out).magnitude, 0
                )
                ...
                for i in range(N_lines_n_n):
                    if i == 0:
                        count += 1
                    n_sheet.cells(count, 3).value = round(
                        Qs_n_n[i, j].to(flow_unit_out).magnitude, 1
                    )
                    n_sheet.cells(count, 4).value = round(
                        H_n_n[i, j].to(var1_unit_out).magnitude, 1
                    )
                    if var2_type_out == "[efficiency]":
                        if var2_symbol_out == "%":
                            n_sheet.cells(count, 5).value = int(
                                round(Eff_n_n[i, j].magnitude * 100, 0)
                            )
                        else:
                            n_sheet.cells(count, 5).value = round(
                                Eff_n_n[i, j].magnitude * 100, 1
                            )
                    count += 1
                    if i == (N_lines - 1):
                        count += 1
            ...
            palette = cycle(
                ["darkslateblue", "mediumblue", "dodgerblue", "orangered", "red"]
            )
            color = list()
            soma = N_speed
            ...
            for i in range(N_speed_n_n):
                ...
                Data_compH.append(
                    go.Scatter(
                        x=Qs_n_n[:, i],
                        y=H_n_n[:, i],
                        line=dict(color=color[i_color], width=1, dash="dash"),
                        mode="lines",  # marker=dict(size=4),
                        name=str(int(speed_n_n[i].magnitude))
                        + " "
                        + speed_unit_out
                        + " - "
                        + str(int(round(100 * speed_fraction_n_n[i].magnitude, 0)))
                        + "% N<sub>rated</sub>"
                        + "- converted",
                    )
                )
                ...
                Data_compH.append(
                    go.Scatter(
                        ...
                        marker=dict(
                            color=color[i_color],
                            size=1.5,
                            line=dict(color=color[i_color], width=2),
                        ),
                        name=str(int(speed_o[i].magnitude))
                        + " "
                        + speed_unit_out
                        + " - "
                        + str(int(round(100 * speed_fraction_o[i].magnitude, 0)))
                        + "% N<sub>rated</sub>"
                        + "- design",
                    )
                )
                ...
                for i in range(N2_speed):
                    ...
                    Data_compH.append(
                        go.Scatter(
                            x=Qs2_o[:, i],
                            y=H2_o[:, i],
                            line=dict(color=color[i + N_speed], width=1, dash="dot"),
                            marker=dict(size=4),
                            ...
                            #                 color=color[i_color],
                            #                 width=2)),
                            name=str(int(speed2_o[i].magnitude))
                            + " "
                            + speed_unit_out
                            + " - "
                            + str(int(round(100 * speed2_fraction_o[i].magnitude, 0)))
                            + "% N<sub>rated</sub>"
                            + "- design 2",
                        )
                    )
            ...
            if (
                ...
                Data_compH.append(
                    go.Scatter(
                        ...
                        opacity=0.55,
                        marker=dict(
                            color="gold", size=9, line=dict(width=1.5, color="black")
                        ),
                        mode="markers",
                        name="Test",
                    )
                ...
                if len(index_o) > 0:
                    Data_compH.append(
                        go.Scatter(
                            ...
                            opacity=0.55,
                            marker=dict(
                                color="red", size=9, line=dict(width=1.5, color="black")
                            ),
                            mode="markers",
                            name="Test - design gas",
                        )
                    )
                    Data_compEff.append(
                        go.Scatter(
            ...
            if pr:
                ...
                fig.update_layout(
                    xaxis_title="Q<sub>s</sub> (" + flow_unit_out + ")",
                    yaxis_title="r<sub>c</sub> (" + var1_unit_out + ")",
                    showlegend=True,
                )
            else:
                fig = go.Figure(
                ...
                fig.update_layout(
                    xaxis_title="Q<sub>s</sub> (" + flow_unit_out + ")",
                    yaxis_title="H<sub>p</sub> (" + var1_unit_out + ")",
                    showlegend=True,
                )
            ...
            for i in range(N_speed):
                ...
                Data_H_n.append(
                    go.Scatter(
                        ...
                        line=dict(color=color[i_color], width=1, dash="dash"),
                        mode="lines",  # marker=dict(size=4),
                        name=str(int(speed_n[i].magnitude))
                        + " "
                        + speed_unit_out
                        + " - "
                        + str(int(round(100 * speed_fraction_n[i].magnitude, 0)))
                        + "% N<sub>rated</sub> - converted from design",
                    )
                )
                Data_eff_n.append(
                    go.Scatter(
                ...
                for i in range(N2_speed):
                    ...
                    Data_H_n.append(
                        go.Scatter(
                            ...
                            y=H2_n[:, i],
                            line=dict(
                                color=color[i + N_speed], width=1, dash="longdashdot"
                            ),
                            mode="lines",  # marker=dict(size=4),
                            # line=dict(color=color[i_color],
                            #           width=1, dash='dash'), marker=dict(size=4),
                            name=str(int(speed2_n[i].magnitude))
                            + " "
                            + speed_unit_out
                            + " - "
                            + str(int(round(100 * speed2_fraction_n[i].magnitude, 0)))
                            + "% N<sub>rated</sub> - converted from design 2",
                        )
                    )
                    Data_eff_n.append(
                        go.Scatter(
            ...
            if desired_speeds:
                    ...
                    for i in range(N_speedv):
                        ...
                        Data_H_n_n.append(
                            go.Scatter(
                                ...
                                name=str(int(speed_v[i].magnitude))
                                + " "
                                + speed_unit_out
                                + " - "
                                + str(
                                    int(round(100 * speed_fraction_v[i].magnitude, 0))
                                )
                                + "% N<sub>rated</sub>"
                                + "- vendor curve",
                            )
                ...
                for i, fig in enumerate(figs_n_n):
                    ...
                    fig.update_layout(width=w, height=h)
                    try:
                        graf_n_.pictures.add(fig, top=i * (h - 100)).pictures.add(
                            fig, top=i * (h - 100)
                        )
                    except:
                        pass
