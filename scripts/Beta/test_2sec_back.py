...
if __name__ == '__main__':
    ...
    if AT_sheet["U6"].value != None:
        ...
        if Nc > 0 and min(abs(QFD.to(flow1_unit).m - flow_v_FD.to(flow1_unit).m)) == 0:
            Gar = [None, None, None, None]
            Curva[Nc, :].value = Gar

        else:
            Gar = [
                flow_v_FD.to(flow1_unit).magnitude,
                P_FD.head.to(head1_unit).magnitude,
                None,
                P_FD.eff.magnitude,
            ]
            Curva[Nc, :].value = Gar
        ...
        if Nc2 > 0 and min(abs(QFD2.to("m³/h").m - P2_FD.flow_v.to(flow2_unit).magnitude)) == 0:
            ...
            Gar = [
                P2_FD.flow_v.to(flow2_unit).magnitude,
                P2_FD.head.to(head2_unit).magnitude,
                None,
                P2_FD.eff.magnitude,
            ]
            Curva2[Nc2, :].value = Gar
...
###################continuar daqui ###########################################
    ...
    if TP_sheet["R19"].value != None:
        ...
        ################################################################################################################
        ###############################Check test procedure from this point#############################################
        ################################################################################################################
