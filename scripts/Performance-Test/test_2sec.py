...
if __name__ == "__main__":
    ...
    while Open_file:
        ...
        if AT_sheet["T7"].value != None:
            ...
            QFD = Q_(np.array(Curva[0:Nc, 0].value), FD_sheet["AP41"].value)

            if Nc > 0 and min(abs(QFD.to("m³/h").m - flow_v_FD.to("m³/h").m)) == 0:
            ...
            for i in range(N):
                ...
                if SS_config == "IN":
                    ...
                    TSS_AT = Td_AT
                # else:
                #     Ps2_AT = Q_(Dados_AT[i, 8].value, AT_sheet.range("O6").value)
                #     Ts2_AT = Q_(Dados_AT[i, 8].value, AT_sheet.range("O6").value)
